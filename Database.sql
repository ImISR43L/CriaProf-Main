...
-- ========= PASSO 1: LIMPEZA DAS TABELAS DE PERGUNTAS ANTIGAS =========
-- Apaga as tabelas na ordem correta para evitar erros de chave estrangeira.
-- A opção CASCADE remove automaticamente políticas e dependências.
DROP TABLE IF EXISTS public.questions CASCADE;
DROP TABLE IF EXISTS public.template_questions CASCADE;


-- ========= PASSO 2: RECRIAÇÃO DA ESTRUTURA DE PERGUNTAS E OPÇÕES =========

-- Tabela 2.1: questions (para atividades dos utilizadores)
CREATE TABLE public.questions (
  id bigint GENERATED BY DEFAULT AS IDENTITY,
  quiz_id uuid NOT NULL,
  "text" text NULL,
...
-- Tabela 2.2: answer_options (para atividades dos utilizadores)
...
COMMENT ON TABLE public.answer_options IS 'Stores answer options for each question.';


-- Tabela 2.3: template_questions (para templates)
CREATE TABLE public.template_questions (
  id bigint GENERATED BY DEFAULT AS IDENTITY,
  template_id uuid NOT NULL,
  "text" text NULL,
  ...
  correct_option_id bigint NULL,
  CONSTRAINT template_questions_pkey PRIMARY KEY (id),
  CONSTRAINT template_questions_template_id_fkey FOREIGN KEY (template_id) REFERENCES templates(id) ON DELETE CASCADE
);
COMMENT ON TABLE public.template_questions IS 'Stores questions for pre-made templates.';


CREATE TABLE public.template_answer_options (
  id bigint GENERATED BY DEFAULT AS IDENTITY,
  question_id bigint NOT NULL,
  "text" text NULL,
  answer text NOT NULL,
  ...
  CONSTRAINT template_answer_options_question_id_fkey FOREIGN KEY (question_id) REFERENCES template_questions(id) ON DELETE CASCADE
);
COMMENT ON TABLE public.template_answer_options IS 'Stores answer options for template questions.';

ALTER TABLE public.questions ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.answer_options ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.template_questions ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.template_answer_options ENABLE ROW LEVEL SECURITY;
...
CREATE POLICY "Users can view options of own or public quizzes." ON public.answer_options FOR SELECT USING ( (SELECT user_id FROM public.quizzes WHERE id = (SELECT quiz_id FROM questions WHERE id = answer_options.question_id)) = auth.uid() OR (SELECT is_public FROM public.quizzes WHERE id = (SELECT quiz_id FROM questions WHERE id = answer_options.question_id)) = true );
CREATE POLICY "Users can create options for their own quizzes." ON public.answer_options FOR INSERT WITH CHECK ( (SELECT user_id FROM public.quizzes WHERE id = (SELECT quiz_id FROM questions WHERE id = answer_options.question_id)) = auth.uid() );
CREATE POLICY "Users can update options of their own quizzes." ON public.answer_options FOR UPDATE USING ( (SELECT user_id FROM public.quizzes WHERE id = (SELECT quiz_id FROM questions WHERE id = answer_options.question_id)) = auth.uid() );
CREATE POLICY "Users can delete options of their own quizzes." ON public.answer_options FOR DELETE USING ( (SELECT user_id FROM public.quizzes WHERE id = (SELECT quiz_id FROM questions WHERE id = answer_options.question_id)) = auth.uid() );

CREATE POLICY "Template questions are viewable by everyone." ON public.template_questions FOR SELECT USING (true);
CREATE POLICY "Admins can manage template questions." ON public.template_questions FOR ALL USING (is_admin()) WITH CHECK (is_admin());

CREATE POLICY "Template answer options are viewable by everyone." ON public.template_answer_options FOR SELECT USING (true);
CREATE POLICY "Admins can manage template answer options." ON public.template_answer_options FOR ALL USING (is_admin()) WITH CHECK (is_admin());
...
CREATE INDEX IF NOT EXISTS idx_template_answer_options_question_id ON public.template_answer_options (question_id);
...
ALTER FUNCTION public.log_activity(p_action text, p_entity_type text, p_entity_id uuid, p_details jsonb) SET search_path = public;
ALTER FUNCTION public.duplicate_questionnaire(p_questionnaire_id uuid) SET search_path = public;
ALTER FUNCTION public.get_user_stats() SET search_path = public;
ALTER FUNCTION public.clear_questionnaire_grid(p_questionnaire_id uuid) SET search_path = public;
ALTER FUNCTION public.get_complete_questionnaire(p_questionnaire_id uuid) SET search_path = public;
ALTER FUNCTION public.save_complete_questionnaire(jsonb, jsonb, jsonb, jsonb) SET search_path = public;

-- Otimiza a política de atualização de perfis
DROP POLICY "Users can update their own profile." ON public.profiles;
CREATE POLICY "Users can update their own profile."
ON public.profiles FOR UPDATE
USING ( (select auth.uid()) = id );

-- Otimiza as políticas da tabela 'quizzes'
DROP POLICY "Users can view their own or public quizzes." ON public.quizzes;
CREATE POLICY "Users can view their own or public quizzes."
ON public.quizzes FOR SELECT
USING ( ((select auth.uid()) = user_id) OR (is_public = true) );

DROP POLICY "Users can create quizzes." ON public.quizzes;
CREATE POLICY "Users can create quizzes."
ON public.quizzes FOR INSERT
WITH CHECK ( (select auth.uid()) = user_id );

DROP POLICY "Users can update their own quizzes." ON public.quizzes;
CREATE POLICY "Users can update their own quizzes."
ON public.quizzes FOR UPDATE
USING ( (select auth.uid()) = user_id );

DROP POLICY "Users can delete their own quizzes." ON public.quizzes;
CREATE POLICY "Users can delete their own quizzes."
ON public.quizzes FOR DELETE
USING ( (select auth.uid()) = user_id );

-- Otimiza as políticas da tabela 'questions'
DROP POLICY "Users can view questions of own or public quizzes." ON public.questions;
CREATE POLICY "Users can view questions of own or public quizzes."
ON public.questions FOR SELECT
USING ( (SELECT user_id FROM public.quizzes WHERE id = questions.quiz_id) = (select auth.uid()) OR (SELECT is_public FROM public.quizzes WHERE id = questions.quiz_id) = true );

DROP POLICY "Users can create questions for their own quizzes." ON public.questions;
CREATE POLICY "Users can create questions for their own quizzes."
ON public.questions FOR INSERT
WITH CHECK ( (SELECT user_id FROM public.quizzes WHERE id = questions.quiz_id) = (select auth.uid()) );

DROP POLICY "Users can update questions of their own quizzes." ON public.questions;
CREATE POLICY "Users can update questions of their own quizzes."
ON public.questions FOR UPDATE
USING ( (SELECT user_id FROM public.quizzes WHERE id = questions.quiz_id) = (select auth.uid()) );

DROP POLICY "Users can delete questions of their own quizzes." ON public.questions;
CREATE POLICY "Users can delete questions of their own quizzes."
ON public.questions FOR DELETE
USING ( (SELECT user_id FROM public.quizzes WHERE id = questions.quiz_id) = (select auth.uid()) );

-- Otimiza as políticas da tabela 'answer_options'
DROP POLICY "Users can view options of own or public quizzes." ON public.answer_options;
CREATE POLICY "Users can view options of own or public quizzes."
ON public.answer_options FOR SELECT
USING ( (SELECT user_id FROM public.quizzes WHERE id = (SELECT quiz_id FROM questions WHERE id = answer_options.question_id)) = (select auth.uid()) OR (SELECT is_public FROM public.quizzes WHERE id = (SELECT quiz_id FROM questions WHERE id = answer_options.question_id)) = true );

DROP POLICY "Users can create options for their own quizzes." ON public.answer_options;
CREATE POLICY "Users can create options for their own quizzes."
ON public.answer_options FOR INSERT
WITH CHECK ( (SELECT user_id FROM public.quizzes WHERE id = (SELECT quiz_id FROM questions WHERE id = answer_options.question_id)) = (select auth.uid()) );

DROP POLICY "Users can update options of their own quizzes." ON public.answer_options;
CREATE POLICY "Users can update options of their own quizzes."
ON public.answer_options FOR UPDATE
USING ( (SELECT user_id FROM public.quizzes WHERE id = (SELECT quiz_id FROM questions WHERE id = answer_options.question_id)) = (select auth.uid()) );

DROP POLICY "Users can delete options of their own quizzes." ON public.answer_options;
CREATE POLICY "Users can delete options of their own quizzes."
